...
const rival = getTeamById(team.vs);

  const titulo = document.getElementById('titulo');
  titulo.innerHTML=`Clásico - ${team.name} vs ${rival.name}`

  const container = document.getElementById('images-container');

  const team1 = document.createElement('img');
  team1.src = `https://lh3.googleusercontent.com/d/${team.image}=s311`;
  team1.alt = team.name;

  const vs = document.createElement('img');
  vs.src = `imagenes/vs.png`;
  vs.alt = "VS";

  const team2 = document.createElement('img');
  team2.src = `https://lh3.googleusercontent.com/d/${rival.image}=s311`;
  team2.alt = rival.name;

  container.appendChild(team1);
  container.appendChild(vs);
  container.appendChild(team2);
  
  const containerhistorial = document.getElementById('historial');
  containerhistorial.innerHTML=team.description
}
